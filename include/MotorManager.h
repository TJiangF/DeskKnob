...
public:
    enum ControlMode {
        AngleControl,
        Infinite_TorqueControl,
        Uninfinite_TorqueControl
    };
    MotorManager(MagneticSensorI2C& sensor, int positions = 8, float stiffness = 5.0f);
    void init();                                // 初始化电机
    void updateControlMode(ControlMode mode, int Gear_limit = 0);     // 切换控制模式
    ControlMode getControlMode() const;                      // 切换控制模式
...
private:
    ...
    int max_gear;                              // the position the knob is at
